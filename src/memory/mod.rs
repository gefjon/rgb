...
pub struct Memory {
    ...
    other_internal_ram_banks: [InternalRamBank; 7],
    active_ram_bank_index: usize,
    object_attribute_memory: [d8; 0xa0],
    hardware_io_regs: [d8; 0x80],
    zero: d8, // Lifetime checker won't let me return &d8(Wrapping(0))
    enable_interrupt_flag: d8,
}

impl Memory {
    pub fn index_d8(&self, d16(Wrapping(idx)): d16) -> Option<&d8> {
        ...
        match idx {
            ...
            0xff80 ... 0xfffe => Some(&self.zero),
            0xffff => Some(&self.enable_interrupt_flag),
            _ => unreachable!(),
        }
    }

    pub fn index_mut_d8(&mut self, d16(Wrapping(idx)): d16) -> Option<&mut d8> {
        let idx = idx as usize;
        match idx {
            0x0000 ... 0x00ff => Some(&mut self.restart_and_int_vectors[idx]),
            0x0100 ... 0x014f => None,
            0x0150 ... 0x3fff => None,
            0x4000 ... 0x7fff => None,
            0x8000 ... 0x97ff => Some(&mut self.character_ram[idx - 0x8000]),
            0x9800 ... 0x9bff => Some(&mut self.background_data_0[idx - 0x9800]),
            0x9c00 ... 0x9fff => Some(&mut self.background_data_1[idx - 0x9c00]),
            0xa000 ... 0xbfff => self.cart_ram.as_mut().and_then(|mem| {
                Some(&mut mem[idx - 0xa000])
            }),
            0xc000 ... 0xcfff => Some(&mut self.internal_ram_bank_0[idx - 0xc000]),
            0xd000 ... 0xdfff => Some(&mut self.other_internal_ram_banks[
                self.active_ram_bank_index
            ][idx - 0xd000]),
            0xe000 ... 0xfdff => None,
            0xfe00 ... 0xfe9f => Some(&mut self.object_attribute_memory[idx - 0xfe00]),
            0xfea0 ... 0xfeff => None,
            0xff00 ... 0xff7f => Some(&mut self.hardware_io_regs[idx - 0xff00]),
            0xff80 ... 0xfffe => None,
            0xffff => Some(&mut self.enable_interrupt_flag),
            _ => unreachable!(),
        }
    }
}
